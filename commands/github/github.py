...
class GitHub():
    ...
    def get_authentication_from_config():
        (username, stderr) = Popen(["git","config","--global", "github.user"], stdout=PIPE).communicate()
        (token, stderr) = Popen(["git","config","--global", "github.token"], stdout=PIPE).communicate()
    ...
    def setup_config():
        ...
        Popen(["git", "config", "--global", "github.user", username])
        Popen(["git", "config", "--global", "github.token", token])
    ...
    def get_project(names=[]):
        if not names:
            (project, stderr) = Popen(["git", "remote", "get-url", "origin"],
                                      stdout=PIPE, stderr=PIPE).communicate()
            if not project:
                (project, stderr) = Popen(["git", "remote", "get-url", "gh"],
                                          stdout=PIPE, stderr=PIPE).communicate()
        else:
            for name in names:
                (project, stderr) = Popen(["git", "remote", "get-url", name],
                                          stdout=PIPE, stderr=PIPE).communicate()
                if project:
                    break
    ...
    def _request(self, rest, params=None, method="GET", page=False):
        ...
        response = False
        try:
            base64string = base64.encodestring('%s:%s' % (self.username, self.token)).replace('\n', '')
        ...
        if response:
            ...
            if page and links:
                ...
                if "next" in link and "last" in link:
                    second_response = self._request(link["next"], method=method, page=link["next"] != link["last"])
                try:
    ...
    def repo_create(self, repo, private, description="", org=None):
        ...
        if org is not None:
            url = "orgs/"+org+"/repos"
        created = self._request(url, method="POST", params={"name":repo, "private":private, "description":description})
        return created
    ...
    def repo_collab(self, repo, collab, remove=False):
        ...
        if remove:
            method="DELETE"
        collab = self._request("repos/"+repo+"/collaborators/"+collab, method=method)
        return collab
    ...
    def key_create(self, title, key):
        created = self._request("user/keys", params={"title":title, "key":key, "wiki":False}, method="POST")
        return created
    ...
    def pull_request(self, repo_name, title, head, base, body):
        ...
        request = self._request("repos/"+repo_name+"/pulls", params=p, method="POST")
        return request
    ...
    def pulls_get(self, repo_name, number):
        pulls = self._request("repos/"+repo_name+"/pulls/"+str(number), method="GET")
        return pulls
    ...
    def pull_comments(self, repo_name, pull):
        comments = self._request("repos/"+repo_name+"/issues/"+pull+"/comments", method="GET", page=True)
        return comments
    ...
    def issue_create(self, repo_name, title, body):
        ...
        request = self._request("repos/"+repo_name+"/issues", params=p, method="POST")
        return request

    def issues_list(self, repo_name):
        issues = self._request("repos/"+repo_name+"/issues", method="GET", page=True)
        return issues
    ...
    def issue_comments(self, repo_name, issue):
        comments = self._request("repos/"+repo_name+"/issues/"+issue+"/comments", method="GET", page=True)
        return comments

    def issue_events(self, repo_name, issue):
        events = self._request("repos/"+repo_name+"/issues/"+issue+"/events", method="GET", page=True)
        return events

    def issue_comment(self, repo_name, issue, body):
        comment = self._request("repos/"+repo_name+"/issues/"+issue+"/comments", {"body":body}, "POST")
        return comment

    def ref_status(self, repo_name, ref):
        status = self._request("repos/"+repo_name+"/commits/"+ref+"/status", {"ref":ref}, method="GET")
        return status
